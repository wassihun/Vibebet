        ...
        for (const idsChunk of chunkedIds) {
            try {
                ...
                for (const match of resp.data.response) {
                    ...
                    // 1. ጨዋታው የተጠናቀቀ መሆኑን ማረጋገጥ (FT, PEN, AET)
                    const isCompleted = ['FT', 'PEN', 'AET'].includes(status);
                    
                    // 2. 🌟 አዲስ: ጨዋታው የተሰረዘ፣ የተቋረጠ ወይም የዘገየ መሆኑን ማረጋገጥ (PST, CANC, ABD) 🌟
                    ...
                    // 🔒 3. ጨዋታው ከተሰረዘ/ከተዘዋወረ የዚያን ጨዋታ ምርጫ ወደ 'won' (በ 1.00 ኦድ) መቀየር (Void/Push Rule)
                    if (isCancelledOrPostponed) {
                        for (let pick of associatedPicks) {
                            await db.query("UPDATE ticket_items SET match_status = ?, score = ? WHERE id = ?", ['won', 'Postponed', pick.id]);
                        }
                        continue;
                    }
